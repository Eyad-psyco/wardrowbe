...
import { DEFAULT_TYPE_ICON, getBuiltinTypeIcon } from '@/lib/type-icons';
...
export function useClothingTypes() {
  ...
  return useMemo(() => [
    ...CLOTHING_TYPES.map((ct) => ({ value: ct.value as string, label: t(ct.value), icon: getBuiltinTypeIcon(ct.value) })),
